...
function App(event) {
  ...
  const handleSubmit = (e, props) => {
      ...
      axios.post("http://127.0.0.1/api/world/create", {
  "name": post,
  "area": 1,
  "pop2005": 1,
  "fips": "1",
...
console.log(error);
  ...
  let [name, setName] = useState("http://127.0.0.1/api/world/");
  ...
  let updateName = (event) => {
    ...
    if (event.key === 'Enter' && event.target.value.slice(-4) != "1914" && event.target.value != "German Empire") {
      ...
      setName("http://127.0.0.1/api/world/" + event.target.value)
    }
    ;

    if (event.key === 'Enter' && event.target.value === "German Empire") {
      event.preventDefault();
      setName("http://127.0.0.1/api/german_empire")
    }
    ;

    if (event.key === 'Enter' && event.target.value.slice(-4) === "1914") {
      event.preventDefault();
      setName("http://127.0.0.1/api/world1914/" + event.target.value.slice(0, -4))
    }
